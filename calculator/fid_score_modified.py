...
import numpy as np
import torch
from inception_score import InceptionV3
...
def get_activations(gen_real_imgs, model, batch_size=50, dims=2048,
    ...
    if gen_real_imgs.shape[0] % batch_size != 0:
        print(('Warning: number of images is not a multiple of the '
               'batch size. Some samples are going to be ignored.'))
    if batch_size > gen_real_imgs.shape[0]:
        print(('Warning: batch size is bigger than the data size. '
               'Setting batch size to data size'))
        batch_size = gen_real_imgs.shape[0]

    n_batches = gen_real_imgs.shape[0] // batch_size

    # normalize
    gen_real_imgs = (gen_real_imgs + 1.0) / 2.0
    pred_arr = []
    for i in tqdm(range(n_batches)):
        ...
        images = gen_real_imgs[start: end]
        model.to("cuda:0")
...
def calculate_activation_statistics(gen_real_imgs, model, batch_size=50,
                                    dims=2048, cuda=False, verbose=False):
    """Calculation of the statistics used by the FID.
    Params:
    -- gen_real_imgs    : gen_real_imgs, tensor
    -- model       : Instance of inception model
    ...
    -- sigma : The covariance matrix of the activations of the pool_3 layer of
               the inception model.
    """
    act = get_activations(gen_real_imgs, model, batch_size, dims, cuda, verbose)
...
def _compute_statistics_of_path(path, model, batch_size, dims, cuda):
    if isinstance(path, str):
        ...
        # a tensor
        gen_real_imgs = path
        m, s = calculate_activation_statistics(gen_real_imgs, model, batch_size,
                                               dims, cuda)

    return m, s


def test_calculate_fid_given_paths_torch(gen_imgs, real_imgs, require_grad=False, batch_size=50, cuda=True, dims=2048):
    """
    Calculates the FID of two paths
    :param gen_imgs: The value range of gen_imgs should be (-1, 1). Just the output of tanh.
    :param real_imgs: real imgs
    :param batch_size:
    :param cuda:
    :param dims:
    :return:
    ...
    # if not os.real_imgs.exists(real_imgs):
    #     raise RuntimeError('Invalid real_imgs: %s' % real_imgs)
    ...
    with _get_no_grad_ctx_mgr(require_grad=require_grad):
        ...
        m2, s2 = _compute_statistics_of_path(real_imgs, model, batch_size,
                                             dims, cuda)
...
modified_real_images = []

def set_real_images(real)\